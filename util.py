...
def initialize_errors(error_options, n_its):
    ...
    return {fun_name: jnp.zeros(n_its) for (fun_name, _) in error_options['error_func_list']}
...
def subspace_error(Uhat, U, relative_error_flag=True):
    ...
    A = Uhat.T.dot(U)
    B = Uhat.T.dot(Uhat)
    err = jnp.sqrt(K + jnp.trace(B.dot(B)) - 2 * jnp.trace(A.dot(A.T)))
...
def get_scale_data_factor(X):
    ...
    norm_fact = jnp.mean(jnp.sqrt(jnp.sum(X ** 2, 0)))
...
def generate_samples(K=None, N=None, D=None, method='spiked_covariance', options=None, scale_data=True,
    ...
    if method == 'spiked_covariance':
        ...
        if normalize:
            ...
            sigma = jnp.sqrt(jnp.linspace(1, lambda_K, K))
        else:
            slope = options['slope']
            gap = options['gap']
            sigma = jnp.sqrt(gap + slope * jnp.arange(K - 1, -1, -1))

        U, _ = jnp.linalg.qr(random.normal(key, (D, K)))

        w = random.normal(key, (K, N))
        X = jnp.sqrt(rho) * random.normal(key, (D, N))

        X += U.dot((w.T * sigma).T)
        sigma2 = (sigma ** 2)[:, jnp.newaxis]
    ...
    if shuffle:
        ...
        X = X[:,random.permutation(key, X.shape[-1])]
